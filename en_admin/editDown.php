...
checkLogined();
$id=$_REQUEST['id'];
$row = fetchOne("select d.pubTime,d.id,p.pName from en_download d
      join en_product p on d.pid=p.id where d.id={$id}");
?>
<!doctype html>
<html>
<head>
	...
	<tr>
		...
		<td>
			<ul style="margin:0;padding:0;">
				<?php
				    $files = fetchAll("select id,filePath from en_file where did={$row['id']}");
