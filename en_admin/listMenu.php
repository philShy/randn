...
// 得到数据库中所有菜单
$sql = "select id,menuName,menucName,location,sort_order,grade,parentId from {$GLOBALS['TB_MENU']} where parentId=0 order by sort_order asc";
$rows = fetchAll ($sql);
		...
		<table class="table" cellspacing="0" cellpadding="0">
			...
			<tbody>
               ...
               <tr>
					...
					<?php $secondMenu = fetchAll("select * from {$GLOBALS['TB_MENU']} where parentId={$row['id']} and grade=2 order by sort_order asc");
					...
					<?php $thirdMenu = fetchAll("select * from {$GLOBALS['TB_MENU']} where parentId={$second['id']} and grade=3 order by sort_order asc");
