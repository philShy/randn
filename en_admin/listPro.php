...
$sql = "select p.id,p.pName,p.rePId,p.price,pAbstract,p.pDesc,p.pStandard,p.pubTime,
        bi.big_cName from en_product as p 
		join en_big_cate bi on p.big_cId=bi.id where 1 {$where}";
...
$sql = "select p.id,p.pName,p.rePId,p.price,pAbstract,p.pDesc,p.pStandard,p.pubTime,
        bi.big_cName from en_product as p 
		join en_big_cate bi on p.big_cId=bi.id where 1 {$where} limit {$offset},{$pageSize}";
$rows=fetchAll($sql);
?>
<!doctype html>
<html>
							...
							<table class="table" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<?php 
										...
										$rePro = fetchAll("select pName from en_product where id in ({$rePId})");
										$pName = "";
