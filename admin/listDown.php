...
$sql="select d.pubTime,p.pName,b.big_cName from download d
      join product p on d.pid=p.id
      join big_cate b on p.big_cId=b.id where 1 {$where}";
...
$sql="select d.pubTime,d.id,p.pName,b.big_cName from download d
      join product p on d.pid=p.id
      join big_cate b on p.big_cId=b.id where 1 {$where} order by id asc limit {$offset},{$pageSize}";
$rows=fetchAll($sql);
      ...
      <tbody>
       <?php  foreach($rows as $row):?>
         ...
         <!--这里的id和for里面的c1 需要循环出来-->
             ...
             <td>
             <?php 
             $file = fetchAll("select filePath from file where did={$row['id']}");
             if (!empty($file)):
             ?>
             <ul>
